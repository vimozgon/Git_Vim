...
class PostItem(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if Feedback.objects.filter(author_id=self.request.user.id).filter(post_id=self.kwargs.get('pk')):
...
class Responses(LoginRequiredMixin, ListView):  #
    ...
    def get_context_data(self, **kwargs):
        ...
        if title:
            ...
            context['filter_responses'] = list(Feedback.objects.filter(post_id=post_id).order_by('-time_public'))
            context['response_post_id'] = post_id.id
        else:
            context['filter_responses'] = list(
                Feedback.objects.filter(post_id__author_id=self.request.user).order_by('-time_public'))
        context['myresponses'] = list(Feedback.objects.filter(author_id=self.request.user).order_by('-time_public'))
        return context
