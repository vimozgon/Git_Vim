...
def respond_send_email(respond_id):
    respond = Feedback.objects.get(id=respond_id)
    send_mail(
...
def respond_accept_send_email(response_id):
    respond = Feedback.objects.get(id=response_id)
...
def send_mail_monday_8am():
    ...
    now = timezone.now()
    list_week_posts = list(Post.objects.filter(time_public__gte=now - timedelta(days=7)))
    if list_week_posts:
        for user in User.objects.filter():
            ...
            send_mail(
                subject=f'MMORPG Billboard: посты за прошедшую неделю.',
